...
def run_pair_walk_forward_window(df, window, lookbacks, entry_thresholds, exit_thresholds, n, cost_rate):
    train_end=window["train_end"]
    validation_end=window["validation_end"]
    test_end=window["test_end"]

    training_results=evaluate_pair_parameters_on_period(df,0,train_end,lookbacks,entry_thresholds,exit_thresholds,cost_rate)
    ...
    test_results=evaluate_pair_period(df,validation_end,test_end,best_lookback,best_entry_threshold,best_exit_threshold,cost_rate)
...
def run_pair_walk_forward(df, lookbacks, entry_thresholds, exit_thresholds, n, cost_rate, initial_train_size, validation_size, test_size):
    ...
    for window in windows:
        result=run_pair_walk_forward_window(df,window,lookbacks,entry_thresholds,exit_thresholds,n,cost_rate)
