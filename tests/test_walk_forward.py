...
from src.analyse import calculate_returns, validate_data
...
def test_generate_walk_forward_windows_rejects_invalid_sizes(
    number_of_rows,
    initial_train_size,
    validation_size,
    test_size
...
def test_run_walk_forward_window():
...
def test_run_walk_forward_window_does_not_modify_input():
    df = make_long_dataframe()
...
def test_run_walk_forward():
    ...
    assert list(combined.index) == list(
        range(25, 40)
    )
...
def test_run_walk_forward_combines_test_periods_in_order():
    df = make_long_dataframe()
...
def test_run_walk_forward_recalculates_cumulative_values():
    df = make_long_dataframe()
    ...
    expected_asset = (
        1 + combined["DailyReturn"].fillna(0)
    ).cumprod()

    expected_strategy = (
        1 + combined["StrategyReturn"]
    ).cumprod()

    expected_net = (
        1 + combined["NetStrategyReturn"]
    ).cumprod()

    assert combined[
        "CumulativeValue"
    ].tolist() == pytest.approx(
        expected_asset.tolist()
    )

    assert combined[
        "StrategyCumulativeValue"
    ].tolist() == pytest.approx(
        expected_strategy.tolist()
    )
...
def test_run_walk_forward_cumulative_values_do_not_reset_between_windows():
    df = make_long_dataframe()
    ...
    first_window_end = combined.iloc[4][
        "NetStrategyCumulativeValue"
    ]

    second_window_start = combined.iloc[5][
        "NetStrategyCumulativeValue"
    ]
    ...
    assert second_window_start == pytest.approx(
        expected_second_start
    )


def test_run_walk_forward_returns_risk_statistics():
    df = make_long_dataframe()
...
def test_run_walk_forward_each_window_has_selected_lookback():
    df = make_long_dataframe()
    ...
    for window_result in result["full_results"]:
        assert window_result["best_lookback"] in window_result[
            "candidate_lookbacks"
        ]


def test_run_walk_forward_does_not_modify_input():
    df = make_long_dataframe()
    original_df = df.copy(deep=True)
