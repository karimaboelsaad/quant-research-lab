...
def test_evaluate_period_uses_warmup_history():
    ...
    result = evaluate_momentum_period(
        df,
        start_position=4,
        end_position=7,
        lookback=2,
...
def test_evaluate_period_rejects_invalid_lookback(
    lookback,
):
    df = make_prepared_dataframe(
        [
    ...
    with pytest.raises(
        ValueError,
        match="Lookback must be positive",
    ):
        evaluate_momentum_period(
            df,
            start_position=0,
...
def test_evaluate_period_rejects_invalid_boundaries(
    start_position,
    end_position,
):
    df = make_prepared_dataframe(
    ...
    with pytest.raises(
        ValueError,
        match="Invalid evaluation period",
    ):
        evaluate_momentum_period(
            df,
...
def test_evaluate_lookbacks_on_period():
    ...
    result = evaluate_lookbacks_on_period(
        df,
        start_position=4,
        end_position=8,
        lookbacks=lookbacks,
    ...
    for lookback in lookbacks:
        period_result = evaluate_momentum_period(
            df,
            start_position=4,
            end_position=8,
            lookback=lookback,
        ...
        actual_return = result.loc[
            result["Lookback"] == lookback,
            "NetStrategyReturn",
        ].iloc[0]

        assert actual_return == pytest.approx(
            expected_stats[
                "net_strategy_return"
            ]
        )
...
def test_select_best_lookback():
    ...
    best_lookback = select_best_lookback(
        results
    )
...
def test_select_top_lookbacks_rejects_non_positive_n(
    invalid_n,
):
    results = pd.DataFrame(
        {
...
def test_run_optimisation_runs_complete_pipeline():
    df = make_long_dataframe()
    ...
    assert result["best_lookback"] in (
        result["candidate_lookbacks"]
    )
...
def test_run_optimisation_returns_complete_test_backtest():
    df = make_long_dataframe()
...
def test_run_optimisation_returns_correct_test_statistics():
    df = make_long_dataframe()
...
def test_run_optimisation_returns_risk_statistics():
    df = make_long_dataframe()
...
def test_run_optimisation_does_not_modify_input():
    df = make_long_dataframe()
...
def test_run_optimisation_rejects_too_many_candidates():
    df = make_long_dataframe()
