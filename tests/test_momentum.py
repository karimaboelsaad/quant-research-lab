...
def run_momentum_pipeline(
    ...
    """Run the complete momentum calculation pipeline."""
...
def test_momentum_signal_calculates_lookback_returns():
    ...
    assert result["LookbackReturn"].iloc[3:].tolist() == (
        pytest.approx(
            [
                0.08,
                -0.1818181818,
...
def test_momentum_signal_handles_positive_zero_and_negative_returns():
    ...
    assert pd.isna(
        result["LookbackReturn"].iloc[0]
    )

    assert result["LookbackReturn"].iloc[1:].tolist() == (
        pytest.approx(
            [
                0.05,
                0.0,
...
def test_transaction_costs_reduce_strategy_value():
    ...
    gross_final_value = (
        df["StrategyCumulativeValue"].iloc[-1]
    )

    net_final_value = (
        df["NetStrategyCumulativeValue"].iloc[-1]
    )
...
def test_save_momentum_output_writes_required_columns(
    tmp_path,
    monkeypatch,
):
...
def test_momentum_pipeline_is_deterministic():
    first_result = run_momentum_pipeline(
        prices=[100, 110, 120, 108, 90, 99],
    )

    second_result = run_momentum_pipeline(
        prices=[100, 110, 120, 108, 90, 99],
    )
...
def test_main_runs_complete_momentum_pipeline(
    tmp_path,
    monkeypatch,
    capsys,
):
